...
> "Every stone remembers how it was reached. To move forward, take one of those 'how I got here' numbers, wiggle it by ±1 (or keep it), and if that lands you on a real stone, write yourself into that stone's logbook."
...
stoneSet     → same positions, but as a Set, for O(1) "is there a stone at X?" checks
dp           → Map<stone_position, Set<jump_sizes_that_landed_here>>
                 "for this stone, which jump sizes got me here?"
stone        → stones[i] — the stone currently being processed (outer loop)
k            → one jump size that's already known to reach `stone` (inner loop, from dp.get(stone))
nextK        → a candidate for the NEXT jump: k-1, k, or k+1
```
